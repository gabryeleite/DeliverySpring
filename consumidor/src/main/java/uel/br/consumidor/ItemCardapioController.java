...
public class ItemCardapioController {
    ...
    public String mostrarPedido(Model model, HttpServletRequest request){
        ...
        // Caso algum item do cardápio tenha sido excluido ou editado
        List<Pedido> sessionPedido = (List<Pedido>)request.getSession().getAttribute(SESSION_PEDIDO);
        if(sessionPedido != null) {
            valorTotal = 0.0;
            Iterator<Pedido> iterator = sessionPedido.iterator();
            while(iterator.hasNext()) {
                Pedido itemPedido = iterator.next();
                ItemCardapio existeItem = cardapioRepository.findById(itemPedido.getId()).orElse(null);
                // caso item do cardapio excluido
                if(existeItem == null) {
                    iterator.remove();
                }
                // caso item do cardapio editado
                if(existeItem != null && !existeItem.equals(itemPedido)) {
                    itemPedido.setNome(existeItem.getNome());
                    itemPedido.setDescricao(existeItem.getDescricao());
                    itemPedido.setPreco(existeItem.getPreco());
                    itemPedido.setNomeRestaurante(existeItem.getRestaurante().getNome());
                }
                // atualiza valorTotal
                if(existeItem != null) {
    ...
    public String adicionarItemPedido (@PathVariable("id") int id, HttpServletRequest request) {
        ...
        Double valorTotal = 0.0; // atualiza valorTotal
        for(Pedido itemPedido : pedido) {
            valorTotal = valorTotal + Double.valueOf(itemPedido.getQuantidade() * itemPedido.getPreco());
            itemPedido.setValorTotal(valorTotal);
        }
